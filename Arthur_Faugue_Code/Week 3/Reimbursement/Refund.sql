...
/
/*
UPDATE EMAIL AND PASSWORD WHERE U_ID = USER.GETID
*/
UPDATE USER_E
SET EMAIL = ?, PASSWRD = ?
WHERE U_ID = ?;
/
SELECT * FROM USER_E WHERE U_ID = 210;
/
/*
USE TO GET ALL REQUEST INFO GET ALL REGULAR INFO FOR REQUEST AND FN & LN
FOR THE REQUESTOR THEN GET THE FN & LN FOR THE RESOLVER FROM THE 
GETEMPLOYEEFROMID()
*/
SELECT * FROM REQUEST INNER JOIN USER_E ON REQUEST.REQUESTER_ID = USER_E.U_ID;
/
/*
GET USER FN & LN FROM USER ID
*/
SELECT FIRSTNAME, LASTNAME FROM USER_E
WHERE U_ID = 300;
/
SELECT * FROM REQUEST INNER JOIN USER_E ON REQUEST.REQUESTER_ID = USER_E.U_ID
WHERE REQUEST.STATE_ID = 2;
/
UPDATE REQUEST SET RESOLVER_ID = 200 
WHERE R_ID = 750;
/
UPDATE REQUEST SET STATE_ID = 2, DATE_RESOLVE = CURRENT_TIMESTAMP WHERE R_ID = 500;
/
SELECT * FROM USER_E;
/
commit;