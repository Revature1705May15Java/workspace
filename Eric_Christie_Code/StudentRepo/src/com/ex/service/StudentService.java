...
public class StudentService {
   ...
   * service layer aka business logic layer handles functionality of getting the
   * most recently used ID and assigning a new student the next value -- should
   * also only accept a unique email address
   * 
  ...
  public Student addStudent(String fName, String lName, String email) {
    try {
      dao.getStudent(email); // throws StudentNotFoundException if no student is
                             // found
      return null;
    } catch (StudentNotFoundException e) {
      // add the student only if there are no existing students with the
      // specified email
